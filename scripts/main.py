...
def list_databases(host, port, user, password, dbname, sort_by, output_file):
    ...
    try:
        ...
        with psycopg2.connect(host=host, port=port, user=user, password=password, dbname=dbname) as conn:
            logging.info("Connected to the main database. Retrieving list of databases...")
            with conn.cursor() as cursor:
                ...
                for db in databases:
                    ...
                    try:
                        with psycopg2.connect(host=host, port=port, user=user, password=password, dbname=dbname) as db_conn:
            ...
            for db_info in db_list:
                db_info.update((k, [dbname] + v if isinstance(v, list) else v) for k, v in db_info.items())
            ...
            summary_table.field_names = ["Database", "Size (MB)", "Table Count", "Row Count"]
            ...
            for index, row in df.iterrows():
                summary_table.add_row([row['Database'], f"{row['Size'] / (1024**2):.2f}", row['TableCount'], row['RowCount']])
            ...
            for db_info in db_list:
                ...
                routine_table.field_names = ["Database", "Schema", "Routine Type", "Routine Name"]
                ...
                for schema, routines in db_info['Routines'].items():
                    for routine_name, routine_type in routines:
                        routine_table.add_row([db_info['Database'], schema, routine_type, routine_name])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sort_by", choices=["alphabet", "size", "tables", "rows"], default="alphabet", help="Sort databases by criteria")
    parser.add_argument("--output_file", help="Output file path")

    args = parser.parse_args()
    
    logging.info("Starting database listing script")
    list_databases(args.db_host, args.db_port, args.db_user, args.db_password, args.db_name, args.sort_by, args.output_file)
